...
users ←──── league_members ────→ leagues
  ...
  └── teams ───────┘                  │
       ...
       ├── rider_xp_daily              (avec role_mult, gt_classif_bonus, gt_distance_bonus NUMERIC(5,1) — stage-hunter breakaway additive bonus, Spec A A3 ; assist_bonus NUMERIC(4,1) — domestique assists réel-équipier, refonte barème rank-based 2026-07 ; kom_event_bonus / sprint_event_bonus numeric — termes additifs cols + sprints intermédiaires, refonte Vuelta 2026, migration 20260828000100)
       ...
       riders → race_results              (cols: breakaway_kms numeric — km en breakaway, NULL si inconnu ; profile_icon text — profil PCS p0-p5, NULL sur résultats GC)
              ...
              → stage_event_results       (événements in-race des étapes GT : passages de cols (kom, category HC/1/2/3/4) + sprints intermédiaires (sprint, category NULL) ; PK (race_slug, event_type, event_name, rider_id), CHECK rank >= 1 et couplage kom↔category ; peuplée par services/pcs-sync/stage_events.py:import_stage_events (post-race + commande import-events, ré-import = delete-by-slug puis upsert) ; lue par les termes kom/sprint_event_bonus du scoring — migration 20260828000000, refonte Vuelta 2026)
...
| **Backfill events** | `import-events --race <stage-slug>` | Rattrapage (post-race l'appelle déjà) | ~10s |
...
- **Refonte Vuelta 2026 (2026-08)** : boost Underdog restreint aux rank_points (dans la parenthèse, plus sur les bonus additifs) ; termes additifs `kom_event_bonus` / `sprint_event_bonus` (lus depuis `stage_event_results`, hors strategy_bonus et boost Underdog, sous `nemesis_modifier`) avec garde anti-silence sur les étapes p4/p5 sans import KOM ; barèmes finaux GT rehaussés (GC 450, Points/KOM 150, Youth 75). La formule est miroitée dans `scripts/verify_tdf2026_closeout.py` et `scripts/dryrun_vuelta2026_rescore.py` — tout changement de formule = 3 édits synchrones.
