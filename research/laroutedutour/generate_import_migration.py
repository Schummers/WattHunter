...
def main() -> None:
    ...
    w("")
    w("-- Written by migrations only. NOTE: the `anon` grant below is narrowed to")
    w("-- `authenticated` by migration 20260914000300 — this is the private history")
    w("-- of one group under their real account names, and the demo league is the")
    w("-- public shop window. This file is left as it was applied in production")
    w("-- rather than rewritten, so local and remote never drift.")
