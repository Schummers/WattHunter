...
export function RaceFeedPhaseEndBanner({ leagueId, nextPhaseRound1Date, nextPhaseLabel }: Props) {
  if (!nextPhaseRound1Date) {
    return (
      <div className="rounded-[var(--radius-compound)] border border-[var(--accent-default)]/30 bg-[var(--bg-surface)] px-3.5 py-3.5">
    ...
    <div className="rounded-[var(--radius-compound)] border border-[var(--accent-default)]/30 bg-[var(--bg-surface)] px-3.5 py-3.5">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link
          ...
          className="shrink-0 rounded-[var(--radius-md)] border border-[var(--accent-default)]/40 bg-[var(--badge-bg)] px-3 py-2 text-[length:var(--type-caption)] font-semibold text-[var(--accent-default)] hover:bg-[var(--badge-bg-hover)] transition-colors"
        >
          View auction →
        </Link>
      </div>
