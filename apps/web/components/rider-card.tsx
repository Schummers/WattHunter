...
export function RiderCard({
  ...
  const inner = (
    <div
      className={`flex items-center gap-3 px-4 py-3 border-b border-[var(--border-subtle)] ${bgClass} ${
        isMuted ? "opacity-60" : ""
      }`}
    >
      {/* Avatar + PCS rank */}
      <div className="flex flex-col items-center gap-0.5">
        <Avatar className="h-9 w-9">
          {rider.photo_url && (
            <AvatarImage src={rider.photo_url} alt={rider.name} />
          )}
        ...
        {rider.pcs_rank != null && (
          <span className="text-[9px] font-bold text-[var(--text-mid)] bg-[var(--bg-surface)] rounded px-1">
            #{rider.pcs_rank}
          </span>
        )}
      </div>
