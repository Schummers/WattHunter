...
async function renderDemoAuctionDetail(auctionId: string) {
  ...
  const [
    ...
    supabase
      ...
      .select("id, rider_id, team_id, amount, round, status")
      .eq("auction_id", auctionId),
    supabase.from("contracts").select("rider_id").eq("status", "active").eq("league_id", DEMO_LEAGUE_ID),
    supabase
      .from("contracts")
      .select("rider_id, available_from")
      .eq("status", "released")
      ...
      .gt("available_from", new Date().toISOString()) as unknown as Promise<{ data: { rider_id: string; available_from: string }[] | null; error: unknown }>,
    supabase.from("contracts").select("locked_salary").eq("status", "active").eq("team_id", teamId),
    supabase.from("team_sponsors").select("sponsors(monthly_budget)").eq("team_id", teamId).maybeSingle(),
  ]);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-[length:var(--type-body)] text-[var(--text-mid)]">
          ...
          <span>·</span>
          <Link
            href={`/league/${DEMO_LEAGUE_SLUG}/auction/${auctionId}/results`}
            className="text-[var(--accent-default)] hover:underline"
          >
            View results
          </Link>
