...
describe("Level gating consistency", () => {
  it("all 10 levels have unique poolMin values", () => {
  ...
  it("level 1 pool starts at 351", () => {
    expect(getLevelByNumber(1).poolMin).toBe(351);
  });

  it("level 10 pool starts at 1 (all riders)", () => {
    expect(getLevelByNumber(10).poolMin).toBe(1);
  });
  ...
  it("boundary: level 11 clamps to level 10", () => {
    expect(getLevelByNumber(11).poolMin).toBe(getLevelByNumber(10).poolMin);
  });
});
