-- Fix: confirm_phase_setup — remove references to non-existent columns:
--   sponsors.first_phase_budget and team_sponsors.payments_count (planned but never created).
-- Both caused a silent crash for all non-late-joiner teams during triggerPhasePayday.
-- Simplified: always use sponsors.monthly_budget.
