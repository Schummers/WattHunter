-- Migration: confirm_phase_setup — add late-joiner guard + service_role support
-- Drops old 3-arg signature and replaces with 4-arg (adds p_phase_start timestamptz).
-- Late joiners (team.created_at >= p_phase_start) skip sponsor credit + salary deduction.
-- Auth made optional so service_role (cascade Task 7) can call without auth.uid().

-- Drop old 3-arg signature so callers must update
...
CREATE OR REPLACE FUNCTION public.confirm_phase_setup(
  ...
  p_phase_start timestamptz    -- NEW: used for late-joiner guard
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
