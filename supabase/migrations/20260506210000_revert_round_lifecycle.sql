-- Hotfix: revert round lifecycle side effects introduced in 20260508000000_round_lifecycle.sql
--
-- Changes reverted:
--   1. validate_round: remove steps 11-12 (closing current auction + opening next).
--      validate_round must only convert draft_bids → auction_bids. Auction closing
--      is exclusively the Python resolve pipeline's responsibility.
--   2. place_bid: restore closes_at deadline check removed by the same migration.
--
-- The FOR UPDATE lock on the auction row (added in round_lifecycle) is intentionally kept.

-- ============================================================
-- RPC validate_round (reverted — no auction lifecycle management)
-- ============================================================
...
-- ============================================================
-- RPC place_bid (restored closes_at deadline check)
-- ============================================================
...
BEGIN
  ...
  -- 3. Lookup auction + verify open + verify deadline not passed
