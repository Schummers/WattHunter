-- Migration: 20260529000003_join_rejects_demo.sql
-- Purpose: add is_demo guard to join_league_by_code so that demo leagues
--          can never accept new members (returns {"ok":false,"error":"league_is_demo"}).
-- Only changes from previous version:
--   1. is_demo added to the SELECT from public.leagues
--   2. One IF block inserted after the NOT FOUND check
