-- Migration: 20260703000000_join_league_classic_late_join.sql
-- Purpose: make the LATE-JOIN branch of join_league_by_code mode-aware.
--
-- The late-joiner onboarding (join an already-active league) was written for
-- MANAGER mode: it seeds the new team with the league's average level, average
-- treasury and average XP. That is wrong for CLASSIC mode, where every team runs
-- on a FLAT footing (level 8, 2 000 000 per phase). Only the XP average makes
-- sense (it drops the newcomer into the middle of the standings).
--
-- Classic late-join now seeds:
--   level         = 8            (classicTeamDefaults / starting_level)
--   treasury      = 2 000 000    (CLASSIC_PHASE_BUDGET / classic_phase_reset v_budget)
--   cumulative_xp = league average (unchanged, both modes)
--   created_at    = league.created_at
--
-- The created_at back-date is deliberate: place_bid's late-joiner gate
-- (20260518000002) blocks a team whose created_at is after the current phase's
-- Round 1 close. That penalty is a manager-mode concept and must not apply to a
-- classic newcomer, who is a full member with a flat 2M budget. Anchoring
-- created_at to the league creation time makes the gate a no-op for them.
--
-- Manager mode is byte-for-byte unchanged. Standard-join branch untouched.
