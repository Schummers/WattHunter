-- Auto-release the contract when a DNF refund is claimed.
-- The rider is "evacuated" — user gets 50% back but loses the rider with the standard 5-day cooldown.

CREATE OR REPLACE FUNCTION public.gt_claim_dnf_refund(p_gt_squad_id uuid, p_contract_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Release the contract (5-day cooldown, mirrors release_rider)
  ...
  DELETE FROM public.draft_bids WHERE rider_id = v_rider_id AND team_id = v_team_id;
