-- Co-Unlock: dynamic threshold proportional to league size.
--
-- Previously the co-unlock gate required a fixed 2 teams at a rider's required
-- level. This made the gate too weak in large leagues and risked permanent
-- lockout if it were raised to a fixed 3 in small leagues.
--
-- New rule: a rider unlocks once `required = GREATEST(2, CEIL(0.30 * team_count))`
-- teams in the league have reached the required level. Floor 2 keeps small
-- leagues unchanged; 30% scales the gate up with league size and can never
-- exceed the team count (no permanent lockout).
--
-- MUST mirror apps/web/lib/co-unlock.ts requiredTeamsToUnlock(). See GAME_RULES §12.2.
--
-- This redefines place_bid from its live version (20260518000002); only the
-- co-unlock block (step 8) changed.
