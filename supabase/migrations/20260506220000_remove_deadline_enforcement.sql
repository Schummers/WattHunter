-- Remove closes_at deadline enforcement from place_bid.
-- closes_at remains in the DB and UI for display only.
-- The commissioner manually triggers resolve when all players are ready.
...
BEGIN
  ...
  END IF;
  -- closes_at is informational only — no deadline enforcement
  ...
  IF v_existing_bid_id IS NULL THEN
    ...
    SELECT
      (SELECT count(*) FROM public.contracts WHERE team_id = v_team.id AND status = 'active')
      + (SELECT count(*) FROM public.auction_bids WHERE team_id = v_team.id AND status = 'active')
    INTO v_used_slots;
  ...
  IF v_existing_bid_id IS NOT NULL THEN
    UPDATE public.auction_bids SET amount = p_amount, placed_at = now() WHERE id = v_existing_bid_id;
