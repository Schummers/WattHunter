-- Migration: 20260703000200_join_league_classic_standard_join.sql
-- Purpose: make the STANDARD-JOIN branch of join_league_by_code mode-aware.
--
-- Context: a classic-mode team must ALWAYS start a phase with a flat 2 000 000
-- budget and level 8, no matter how or when the player enters the league. The
-- late-join branch was fixed for this in 20260703000000. But the standard-join
-- branch (join a league that is still 'pending', not yet launched) was left as
-- manager logic: it inserts a team WITHOUT a treasury, so the row falls back to
-- the column default (0). A player who joins a pending classic league by code
-- would therefore sit at 0 for the first draft, only topped up to 2M at the
-- first classic_phase_reset (which fires at a phase transition, not at join).
--
-- Fix: in the standard-join branch, seed classic teams with
--   level         = 8            (classicTeamDefaults / starting_level)
--   treasury      = 2 000 000    (CLASSIC_PHASE_BUDGET / classic_phase_reset v_budget)
--   underdog_eligible = false
-- Manager mode standard-join is byte-for-byte unchanged (no treasury set ->
-- column default, as before). The late-join branch is unchanged from
-- 20260703000000.
...
BEGIN
  ...
  IF v_league.status != 'active' THEN
    IF v_league.mode = 'classic' THEN
      -- Classic: flat footing from the start. Level 8 + 2M, like every team.
      v_start_level := 8;

      INSERT INTO public.teams (league_id, user_id, name, level, cumulative_xp, treasury, underdog_eligible)
        VALUES (
          v_league.id, v_user_id,
          COALESCE(NULLIF(TRIM(p_team_name), ''), 'My Team'),
          v_start_level, 0, 2000000, false
        )
        RETURNING id INTO v_team_id;
    ELSE
      -- Manager: start at the league's starting level, treasury column default.
  ...
  -- XP average is used in both modes (drops the newcomer mid-standings).
  SELECT
  ...
  IF v_league.mode = 'classic' THEN
    -- Classic: flat footing. Level and treasury are fixed, not averaged.
    -- created_at back-dated to league creation so place_bid's late-joiner
    -- gate does not exclude them from the current/next phase draft.
    v_start_level := 8;
    ...
    INSERT INTO public.teams (league_id, user_id, name, level, cumulative_xp, treasury, created_at, underdog_eligible)
      ...
      RETURNING id INTO v_team_id;
  ELSE
    -- Manager: derive level from average XP, use averaged treasury (unchanged).
