-- Incrément d'enchère 100 € → 1 000 €, côté RPC.
--
-- Recrée place_bid (depuis 20260602000000_co_unlock_dynamic_threshold.sql) et
-- gt_place_emergency_bid (depuis 20260521000000_gt_rescue_window_refinement.sql)
-- en changeant UNIQUEMENT la garde d'incrément `% 100` → `% 1000` (+ message).
-- Tout le reste de la logique (co-unlock, solvabilité, gating, fenêtre rescue)
-- est inchangé.
...
-- ============================================================
-- gt_place_emergency_bid (GT Rescue) — même changement d'incrément.
-- Recréé depuis 20260521000000_gt_rescue_window_refinement.sql ; seule la
-- garde `% 100` → `% 1000` change.
-- ============================================================
...
BEGIN
  ...
  -- Replace window check. Refund reste anytime, mais replace est gated au 1er rest day.
  ...
  -- Amount validation
  IF p_amount < 5000 OR p_amount % 1000 != 0 THEN
    RETURN jsonb_build_object('error', 'amount must be >= 5000 and a multiple of 1000');
  END IF;
