-- 20260527000000_join_league_by_code_team_name.sql
-- Add optional p_team_name parameter to join_league_by_code.
-- Both INSERT statements now use COALESCE(NULLIF(TRIM(p_team_name), ''), 'My Team')
-- so callers that omit the argument fall back to the default 'My Team'.
-- All existing logic is preserved unchanged.

CREATE OR REPLACE FUNCTION public.join_league_by_code(p_code text, p_team_name text DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
