...
-- Local-portability guard (audit 2026-06-25). The 8 players are real prod auth.users.
-- On a fresh/local DB they are absent, so a plain INSERT fails leagues_commissioner_id_fkey
-- and aborts `supabase db reset` (violating CLAUDE.md Rule #2). We skip the whole seed when
-- the players are not present. On prod this migration already ran with the users present and
-- will NOT re-run, so prod is unaffected. Idempotent via fixed UUIDs + ON CONFLICT DO NOTHING.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM auth.users WHERE id = 'dc1380b4-4147-4f4a-840b-7b5ed602669f'
  ) THEN
    RAISE NOTICE 'Classic V2 seed skipped: players absent (fresh/local DB)';
    RETURN;
  END IF;
