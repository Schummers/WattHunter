-- Remove the late-joiner early-return guard from confirm_phase_setup.
--
-- Before: teams created after p_phase_start skipped sponsor credit + salary
--         deduction entirely (introduced in 20260509140003, kept in 20260509160000).
--         This caused GoudalEnergies (Phase 4) to keep 71 200 € without payday.
-- After:  all teams go through the full payday (sponsor income + salaries).
--         A late joiner who couldn't bid has 0 active contracts — salary
--         deduction = 0, only sponsor income is credited.
--
-- The p_phase_start parameter is kept in the signature (callers unchanged)
-- but is no longer used by the function body.
